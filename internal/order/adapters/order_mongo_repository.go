...
import (
	...
	"github.com/SInITRS/gorder/order/entity"
...
func (r *OrderRepositoryMongo) coll() *mongo.Collection {
...
func (r *OrderRepositoryMongo) Create(ctx context.Context, order *domain.Order) (created *domain.Order, err error) {
	...
	res, err := r.coll().InsertOne(ctx, write)
	if err != nil {
		return nil, err
	}
	created = order
	created.ID = res.InsertedID.(bson.ObjectID).Hex()
	return created, nil
}
...
func (r *OrderRepositoryMongo) Get(ctx context.Context, id, customerID string) (got *domain.Order, err error) {
	...
	read := &orderModel{}
	mongoID, _ := bson.ObjectIDFromHex(id)
	cond := bson.M{"_id": mongoID}
	if err = r.coll().FindOne(ctx, cond).Decode(read); err != nil {
...
func (r *OrderRepositoryMongo) Update(
	...
	mongoID, _ := bson.ObjectIDFromHex(oldOrder.ID)
