...
func (c *Consumer) handleMessage(ch *amqp.Channel, msg amqp.Delivery, q amqp.Queue) {
	var err error
	logrus.Infof("deliver receive a message from %s, msg=%v", q.Name, string(msg.Body))
	...
	if o.Status != "paid" {
		err = errors.New("order status not paid, cannot deliver")
		return
	}
	deliver(o)
	span.AddEvent(fmt.Sprintf("order_deliver: %v", o))
	if err := c.orderGRPC.UpdateOrder(mqCtx, &orderpb.Order{
		...
		if err = broker.HandleRetry(mqCtx, ch, &msg); err != nil {
			logrus.Warnf("deliver: error handling retry: err=%v", err)
		}
		return
	}
	span.AddEvent("deliver.order.finished.updated")
	logrus.Info("consume success")
}

func deliver(o *Order) {
	logrus.Printf("delivering order: %s", o.ID)
	...
	for {
		select {
		case <-ticker.C:
			logrus.Printf("order %s delivering...", o.ID)
		case <-done:
